...
stroke('white')
                ...
                text(`${player.name}:${player.score}`, (player.distance)- 10, 510);
...
if (player.score > 20){
        ...
        if (player.index !== null) {
            for (var i = 0; i < fruitGroup.length; i++) {
                if (fruitGroup.get(i).isTouching(players)) {
                    ...
                    let sca = player.score += 1;
                    player.updateScore(sca);
                }
    ...
    fill('GreenYellow')
    stroke('white');
       text('Game Ended',450,300)
       text(`${player.name} has won the game`,500,350);
      text('Click restart button to reset the game',550,400)
    }
}